...
var envSchema = external_exports.object({
  PORT: external_exports.number().default(4e3),
  JWT_SECRET: external_exports.string().default("JWT_SECRET_LITTLE_SECRET")
});
...
function createRouterFactory(config2) {
  function createRouterInner(input) {
    ...
    function createLazyLoader(opts) {
      return {
        ref: opts.ref,
        load: once(() => __async(null, null, function* () {
...
function getProcedureAtPath(router2, path) {
  return __async(this, null, function* () {
    const { _def } = router2;
...
function createCallerFactory() {
  return function createCallerInner(router2) {
    ...
    return function createCaller(ctxOrCallback, opts) {
      return createRecursiveProxy((_0) => __async(null, [_0], function* ({ path, args }) {
        const fullPath = path.join(".");
        if (path.length === 1 && path[0] === "_def") return _def;
        const procedure = yield getProcedureAtPath(router2, fullPath);
        let ctx = void 0;
        try {
          ...
          return yield procedure({
            path: fullPath,
            getRawInput: () => __async(null, null, function* () {
              return args[0];
            }),
            ctx,
...
function memo(fn) {
  ...
  let value = sym;
  return {
    read: () => __async(null, null, function* () {
      var _promise;
...
var jsonContentTypeHandler = {
  ...
  parse(opts) {
    return __async(this, null, function* () {
      ...
      const getInputs = memo(() => __async(null, null, function* () {
        let inputs = void 0;
        if (req.method === "GET") {
      ...
      const calls = yield Promise.all(paths.map((path, index) => __async(null, null, function* () {
...
function makeAsyncResource(thing, dispose) {
  ...
  it[Symbol.asyncDispose] = () => __async(null, null, function* () {
    yield dispose();
...
function iteratorResource(iterable) {
  ...
  return makeAsyncResource(iterator, () => __async(null, null, function* () {
    ...
    [Symbol.asyncIterator]() {
      return (0, import_wrapAsyncGenerator$4.default)(function* () {
        try {
          ...
          state = "pending";
          const _finally = _usingCtx$1.a(makeAsyncResource({}, () => __async(null, null, function* () {
            state = "done";
            const errors = [];
            yield Promise.all(Array.from(iterators.values()).map((it) => __async(null, null, function* () {
              try {
                yield it.destroy();
              } catch (cause) {
                errors.push(cause);
...
function sseStreamProducer(opts) {
  ...
  function _generatorWithErrorHandling() {
    _generatorWithErrorHandling = (0, import_wrapAsyncGenerator$1.default)(function* () {
      try {
        yield* __yieldStar((0, import_asyncGeneratorDelegate.default)((0, import_asyncIterator.default)(generator())));
        yield {
          event: RETURN_EVENT,
          data: ""
        };
...
function resolveResponse(opts) {
  return __async(this, null, function* () {
    ...
    const infoTuple = yield run(() => __async(null, null, function* () {
      try {
    ...
    const ctxManager = run(() => {
      ...
      return {
        ...
        create: (info) => __async(null, null, function* () {
    ...
    try {
      ...
      const rpcCalls = info.calls.map((call) => __async(null, null, function* () {
      ...
      if (info.accept === "application/jsonl") {
        ...
        const stream = jsonlStreamProducer((0, import_objectSpread23.default)((0, import_objectSpread23.default)({}, config2.jsonl), {}, {
          maxDepth: Infinity,
          data: rpcCalls.map((res) => __async(null, null, function* () {
      ...
      headers.set("content-type", "application/json");
      const results = (yield Promise.all(rpcCalls)).map((res) => {
...
function createInputMiddleware(parse) {
  const inputMiddleware = function inputValidatorMiddleware(opts) {
    return __async(this, null, function* () {
      let parsedInput;
...
function createOutputMiddleware(parse) {
  const outputMiddleware = function outputValidatorMiddleware(_0) {
    return __async(this, arguments, function* ({ next }) {
...
function getParseFn(procedureParser) {
  ...
  if (isStandardSchema) return (value) => __async(null, null, function* () {
...
function createResolver(_defIn, resolver) {
  const finalBuilder = createNewBuilder(_defIn, {
    resolver,
    middlewares: [function resolveMiddleware(opts) {
      return __async(this, null, function* () {
  ...
  const callerWrapper = (...args) => __async(null, null, function* () {
...
function callRecursive(index, _def, opts) {
  return __async(this, null, function* () {
    try {
      ...
      const result = yield middleware((0, import_objectSpread2$13.default)((0, import_objectSpread2$13.default)({}, opts), {}, {
        meta: _def.meta,
        input: opts.input,
        next(_nextOpts) {
...
// ../../node_modules/.pnpm/@trpc+server@11.4.2_typescript@5.8.3/node_modules/@trpc/server/dist/node-http-Du8akt-R.mjs
...
function incomingMessageToRequest(req, res, opts) {
  var _req$socket2, _req$socket2$once;
  const ac = new AbortController();
  const onAbort = () => {
    var _req$socket, _req$socket$off;
    res.off("close", onAbort);
    (_req$socket = req.socket) === null || _req$socket === void 0 || (_req$socket$off = _req$socket.off) === null || _req$socket$off === void 0 || _req$socket$off.call(_req$socket, "close", onAbort);
    ac.abort();
  };
  res.once("close", onAbort);
  (_req$socket2 = req.socket) === null || _req$socket2 === void 0 || (_req$socket2$once = _req$socket2.once) === null || _req$socket2$once === void 0 || _req$socket2$once.call(_req$socket2, "close", onAbort);
...
function writeResponseBodyChunk(res, chunk) {
  return __async(this, null, function* () {
    if (res.write(chunk) === false) yield new Promise((resolve, reject) => {
      const onError = (err) => {
        reject(err);
        cleanup();
      };
...
function writeResponseBody(opts) {
  return __async(this, null, function* () {
    const { res } = opts;
    try {
      let _a;
    ...
    return new Promise((resolve) => {
      ...
      return handleViaMiddleware(opts.req, opts.res, (err) => {
        run(() => __async(null, null, function* () {
          ...
          const createContext2 = (innerOpts) => __async(null, null, function* () {
            var _opts$createContext;
            return yield (_opts$createContext = opts.createContext) === null || _opts$createContext === void 0 ? void 0 : _opts$createContext.call(opts, (0, import_objectSpread25.default)((0, import_objectSpread25.default)({}, opts), innerOpts));
          });
          const response = yield resolveResponse((0, import_objectSpread25.default)((0, import_objectSpread25.default)({}, opts), {}, {
            req: request,
...
// ../../node_modules/.pnpm/@trpc+server@11.4.2_typescript@5.8.3/node_modules/@trpc/server/dist/adapters/express.mjs
...
function createExpressMiddleware(opts) {
  return (req, res) => {
    ...
    run(() => __async(null, null, function* () {
      ...
      yield nodeHTTPRequestHandler((0, import_objectSpread26.default)((0, import_objectSpread26.default)({}, opts), {}, {
        req,
        res,
        path
      }));
...
// src/utils/error.ts
var TRPCServerError = class _TRPCServerError extends TRPCError {
  constructor(code, message) {
    super({ code, message });
  }
  static internalError(message) {
    return new _TRPCServerError("INTERNAL_SERVER_ERROR", message);
  }
  static notFound(message) {
    return new _TRPCServerError("NOT_FOUND", message);
  }
  static badRequest(message) {
    return new _TRPCServerError("BAD_REQUEST", message);
  }
  static forbidden(message) {
    return new _TRPCServerError("FORBIDDEN", message);
  }
  static unauthorized(message) {
    return new _TRPCServerError("UNAUTHORIZED", message);
  }
  static conflict(message) {
    return new _TRPCServerError("CONFLICT", message);
  }
};
...
var authenticatedProcedure = t.procedure.use(({ ctx, next }) => {
  if (!ctx.user) {
...
var JWT = class {
  static decode(token) {
    try {
      ...
      return import_jsonwebtoken.default.verify(extractedToken || "", env.JWT_SECRET);
    } catch (e) {
      return "";
    }
  }
  static encode(payload, options) {
    return import_jsonwebtoken.default.sign(payload, env.JWT_SECRET, options);
  }
...
// src/router/trpc/index.ts
var trpcRouter = router({
  users: usersRouter,
  auth: authRouter
});

// src/middlewares/trpc-middleware.ts
